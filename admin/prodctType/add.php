...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title)) {
        $title_err = 'Enter Your Product Type Name';
    } else {
        ...
        if ($statement = $pdo->prepare($sql)) {
            ...
            if ($statement->execute()) {
                if ($statement->rowCount() === 1) {
                    $title_err = ' Product Type Name Alrady Exits';
                }
            } else {
    ...
    if (empty($title_err)) {
        $sql = 'INSERT INTO foxic_producttype (ProductType_title, create_at, update_at) VALUE(:ProductType_title, :create_at, :update_at)';
        if ($statement = $pdo->prepare($sql)) {
